...
var respostas_erradas = 0;
  ...
  tag.src = "https://www.youtube.com/watch?v=sXo_ghl7NUQ";
...
(function (root, factory) {
  ...
  Lightbox.prototype.init = function() {
    var self = this;
    // Both enable and build methods require the photo tag to be in the DOM.
    $(document).ready(function() {
      self.enable();
      self.build();
    });
  ...
  Lightbox.prototype.enable = function() {
    var self = this;
    $('photo').on('click', 'a[rel^=lightbox], area[rel^=lightbox], a[data-lightbox], area[data-lightbox]', function(event) {
      self.start($(event.currentTarget));
      return false;
    });
  };
  ...
  Lightbox.prototype.build = function() {
    var self = this;
    $('<div id="lightboxOverlay" class="lightboxOverlay"></div><div id="lightbox" class="lightbox"><div class="lb-outerPhoto"><div class="lb-photo"><img class="lb-image" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" /><div class="lb-nav"><a class="lb-prev" href="" ></a><a class="lb-next" href="" ></a></div><div class="lb-loader"><a class="lb-cancel"></a></div></div></div><div class="lb-dataPhoto"><div class="lb-data"><div class="lb-details"><span class="lb-caption"></span><span class="lb-number"></span></div><div class="lb-closePhoto"><a class="lb-close"></a></div></div></div></div>').appendTo($('photo'));

    // Cache jQuery objects
    this.$lightbox       = $('#lightbox');
    this.$overlay        = $('#lightboxOverlay');
    this.$outerPhoto = this.$lightbox.find('.lb-outerPhoto');
    this.$photo      = this.$lightbox.find('.lb-photo');
    this.$image          = this.$lightbox.find('.lb-image');
    this.$nav            = this.$lightbox.find('.lb-nav');

    // Store css values for future lookup
    this.photoPadding = {
      top: parseInt(this.$photo.css('padding-top'), 10),
      right: parseInt(this.$photo.css('padding-right'), 10),
      bottom: parseInt(this.$photo.css('padding-bottom'), 10),
      left: parseInt(this.$photo.css('padding-left'), 10)
    };
    ...
    this.$outerPhoto.on('click', function(event) {
      if ($(event.target).attr('id') === 'lightbox') {
        self.end();
      }
      return false;
  ...
  Lightbox.prototype.start = function($link) {
    ...
    // Disable scrolling of the page while open
    if (this.options.disableScrolling) {
      $('photo').addClass('lb-disable-scrolling');
    }
  ...
  Lightbox.prototype.changeImage = function(imageNumber) {
    ...
    $('.lb-loader').fadeIn('slow');
    this.$lightbox.find('.lb-image, .lb-nav, .lb-prev, .lb-next, .lb-dataPhoto, .lb-numbers, .lb-caption').hide();

    this.$outerPhoto.addClass('animating');

    // When image to show is preloaded, we send the width and height to sizePhoto()
    var preloader = new Image();
    preloader.onload = function() {
      var $preloader;
      var imageHeight;
      ...
      if (self.options.fitImagesInViewport) {
        ...
        windowWidth    = $(window).width();
        windowHeight   = $(window).height();
        maxImageWidth  = windowWidth - self.photoPadding.left - self.photoPadding.right - self.imageBorderWidth.left - self.imageBorderWidth.right - 20;
        maxImageHeight = windowHeight - self.photoPadding.top - self.photoPadding.bottom - self.imageBorderWidth.top - self.imageBorderWidth.bottom - 120;
      ...
      self.sizePhoto($image.width(), $image.height());
    };
  ...
  // Animate the size of the lightbox to fit the image we are showing
  Lightbox.prototype.sizePhoto = function(imageWidth, imageHeight) {
    var self = this;

    var oldWidth  = this.$outerPhoto.outerWidth();
    var oldHeight = this.$outerPhoto.outerHeight();
    var newWidth  = imageWidth + this.photoPadding.left + this.photoPadding.right + this.imageBorderWidth.left + this.imageBorderWidth.right;
    var newHeight = imageHeight + this.photoPadding.top + this.photoPadding.bottom + this.imageBorderWidth.top + this.imageBorderWidth.bottom;

    function postResize() {
      self.$lightbox.find('.lb-dataPhoto').width(newWidth);
      self.$lightbox.find('.lb-prevLink').height(newHeight);
      self.$lightbox.find('.lb-nextLink').height(newHeight);
      self.showImage();
    }
  ...
  Lightbox.prototype.updateDetails = function() {
    ...
    this.$outerPhoto.removeClass('animating');

    this.$lightbox.find('.lb-dataPhoto').fadeIn(this.options.resizeDuration, function() {
      return self.sizeOverlay();
    });
  };
  ...
  Lightbox.prototype.end = function() {
    ...
    if (this.options.disableScrolling) {
      $('photo').removeClass('lb-disable-scrolling');
    }
  };
